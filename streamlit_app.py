...
try:
    ...
    st.error(f"SYSTEM ERROR: Logic components not found. Ensure cwr_engine.py is present. {e}")
    st.stop()
...
with tab1:
    ...
    if file:
        try:
            ...
            # Scans for Harvest Media OR SourceAudio markers
            df_preview = pd.read_csv(file, header=None, nrows=20)
            h_idx = -1
            
            # Key markers for different schemas
            MARKERS = ["TRACK: TITLE", "LIBRARY: NAME", "SOURCEAUDIO ID", "FILENAME", "TITLE"]
            
            for i, row in df_preview.iterrows():
                # Convert row to a single upper-case string for scanning
                row_str = row.astype(str).str.cat(sep=" ").upper()
                if any(m in row_str for m in MARKERS):
                    h_idx = i; break
            
            if h_idx == -1:
                st.error("Error: Schema not recognized. CSV must contain 'TRACK: TITLE' (Harvest) or 'SourceAudio ID'/'Title' (SourceAudio).")
            else:
                file.seek(0)
...
with tab2:
    ...
    val_file = st.file_uploader("Check existing CWR", type=["V22", "txt"])
    if val_file:
