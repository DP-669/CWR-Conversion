...
def generate_cwr_content(df):
    ...
    for i, row in df.iterrows():
        ...
        for p_idx in range(1, 4):
            ...
            cwr_share = (float(pr_raw) if pd.notna(pr_raw) else 0.0) * 0.5
        ...
        for w_idx in range(1, 4):
            ...
            c_w_share = (float(w_pr_raw) if pd.notna(w_pr_raw) else 0.0) * 0.5
            lines.append(asm.build(Blueprints.SWR, {"t_seq": t_seq, "rec_seq": f"{rec_seq:08d}", "writer_id": f"00000000{w_idx}", "last_name": str(w_last), "first_name": str(get_vessel_col(row, "WRITER", w_idx, "First Name") or ""), "ipi": pad_ipi(get_vessel_col(row, "WRITER", w_idx, "IPI")), "pr_soc": "021", "mr_soc": "099", "sr_soc": "099", "pr_share": fmt_share(c_w_share), "mr_share": "00000", "sr_share": "00000"})); rec_seq += 1
            lines.append(asm.build(Blueprints.SWT, {"t_seq": t_seq, "rec_seq": f"{rec_seq:08d}", "writer_id": f"00000000{w_idx}", "pr_share": fmt_share(c_w_share), "mr_share": "00000", "sr_share": "00000"})); rec_seq += 1
            ...
            if orig_pub_name in pub_map:
                ...
                lines.append(asm.build(Blueprints.PWR, {"t_seq": t_seq, "rec_seq": f"{rec_seq:08d}", "pub_id": p_i['id'], "pub_name": orig_pub_name[:45], "agreement": p_i['agr'], "writer_id": f"00000000{w_idx}", "chain_id": p_i['chain']})); rec_seq += 1
        isrc_v = str(row.get('CODE: ISRC', '')); cd_v = str(row.get('ALBUM: Code', 'RC055'))
        lines.append(asm.build(Blueprints.REC, {"t_seq": t_seq, "rec_seq": f"{rec_seq:08d}", "isrc": isrc_v, "cd_id": cd_v, "source": "CD", "title": "", "label": "RED COLA", "duration": parse_duration(row.get('TRACK: Duration', '0'))})); rec_seq += 1
        lines.append(asm.build(Blueprints.ORN, {"t_seq": t_seq, "rec_seq": f"{rec_seq:08d}", "library": "RED COLA", "cd_id": cd_v, "label": "RED COLA"})); rec_seq += 1
        r_sum += (rec_seq)
    lines.append(asm.build(Blueprints.GRT, {"t_count": f"{t_count:08d}", "r_count": f"{(r_sum+2):08d}"}))
    lines.append(asm.build(Blueprints.TRL, {"t_count": f"{t_count:08d}", "r_count": f"{(r_sum+4):08d}"}))
    return "\n".join(lines)
