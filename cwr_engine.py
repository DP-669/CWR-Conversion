...
class CWREngine:
    def __init__(self):
        # Starting ID at 3610 to match your Bible's sequence
        self.trans_base = 3610 
        self.trans_count = 0
        self.record_count = 0
        self.group_count = 0
    ...
    def make_nwr_block(self, row):
        # Increment transaction based on Bible logic
        current_trans = self.trans_base + (self.trans_count * 2) + 1
        self.trans_count += 1
        lines = []
        ...
        # 1. NWR (Note: Bible uses current_trans then current_trans+1 for some records)
        lines.append(f"NWR{self.pad(current_trans, 8, 'right', '0')}{self.pad(0, 8, 'right', '0')}{self.pad(title, 60)}{self.pad('', 14)}{self.pad(iswc, 11)}")
        
        # 2. SPU - OP
        spu1 = f"SPU{self.pad(current_trans, 8, 'right', '0')}{self.pad(1, 8, 'right', '0')}01{self.pad('', 9, '0')}{self.pad(op_name, 45)}E "
        spu1 += f"{self.pad('', 9)}{self.pad(mapping['ipi'], 11, 'right', '0')}0210500002110000   10000 N{self.pad('', 27)}{self.pad(mapping['agreement'], 14)}PG"
        lines.append(spu1)
        
        # 3. SPU - LUM
        spu2 = f"SPU{self.pad(current_trans, 8, 'right', '0')}{self.pad(2, 8, 'right', '0')}01{self.pad('', 9, '0')}{self.pad(SUB_PUB['name'], 45)}SE"
        ...
        lines.append(f"SWR{self.pad(current_trans, 8, 'right', '0')}{self.pad(4, 8, 'right', '0')}{self.pad(writer_last, 45)}{self.pad(writer_first, 30)}C {self.pad('', 9, '0')}{writer_ipi}021050000990000009900000 N")
        ...
        # PWR Alignment fix: Bible has a very specific gap here
    ...
    def make_trl(self):
        # Record count includes HDR + GRH + Blocks + GRT + TRL
        self.record_count += 2 
